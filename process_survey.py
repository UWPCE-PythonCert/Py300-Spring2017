...
with open("Python 3 2017 Topic Options (Responses) - Form Responses 1.tsv") as infile:
    topics = infile.readline().strip().split('\t')[2:]
    print(topics)
    topics = [line.split('[')[1].rstrip(']') for line in topics]
    data_table = [[pref_map[pref.strip()] for pref in line.split('\t')[2:]] for line in infile]
